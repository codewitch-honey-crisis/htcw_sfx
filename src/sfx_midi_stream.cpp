...
namespace sfx {
const size_t midi_stream::decode_message(bool is_file,stream& in, midi_message* in_out_message) {
    if (in_out_message == nullptr) {
        return 0;
    }
    size_t result = 0;
    int i = in.getch();
    ...
    ++result;
    uint8_t b = (uint8_t)i;
    if(in_out_message->status==0xFF && in_out_message->meta.data!=nullptr) {
        free(in_out_message->meta.data);
        in_out_message->meta.data=nullptr;
    }
    if(in_out_message->status==0xF7 && in_out_message->sysex.data!=nullptr) {
        free(in_out_message->sysex.data);
        in_out_message->sysex.data=nullptr;
    }
    bool has_status = b&0x80;
    // expecting a status byte
    if(!has_status) {
        if(!(in_out_message->status&0x80)) {
            // no status byte in message
            return 0;
        }
    } else {
        in_out_message->status = b;
    }
    switch(in_out_message->type()) {
    ...
    case midi_message_type::song_position:
        if(has_status) {
            if(2!=in.read((uint8_t*)&in_out_message->value16,2)) {
                return 0;
            }
            result+=2;
            return result;
        ...
        ++result;
        in_out_message->lsb(b);
        in_out_message->msb((uint8_t)i);    
        return result;
    ...
    case midi_message_type::song_select:
        if(has_status) {
            ...
            ++result;
            in_out_message->value8 = (uint8_t)i;
            return result;
        } 
        in_out_message->value8 = b;
    ...
    case midi_message_type::system_exclusive:
        {
            ...
            in_out_message->sysex.data = psx;
            in_out_message->sysex.size = sxsz;
            return result;
        }
    case midi_message_type::reset:
        ...
        // this is a meta event
            ...
            ++result;
            in_out_message->meta.type = (uint8_t)i;
        {
            int32_t vl;
            size_t sz=midi_utility::decode_varlen(in,&vl);
            // re-encode it to fill our midi message
            midi_utility::encode_varlen(vl,in_out_message->meta.encoded_length);
            result+=sz;
            if(vl>0) {
